...
public:
    ...
    void release();
    std::vector<PoseDetection> detect(const cv::Mat& bgr_img, float conf_threshold = 0.25f, float region_ratio = 1.0f);
    // Raw detect with preprocessed buffer (for pipeline mode)
    std::vector<PoseDetection> detect_raw(uint8_t* preprocessed_buf, float scale, float pad_left, float pad_top, float conf_threshold = 0.25f, float region_ratio = 1.0f);
